...
| F7-WEB | Completada localmente | Migración `0012_round_war_machine.sql`, controles 1:1 con estados `OPEN`/`PENDING_APPROVAL`/`CLOSED`, decimales exactos, locks, RLS, auditoría, idempotencia y acciones `/cash`; suite cash 8/8, API 47/47, typecheck, builds, Drizzle check, diff check y smoke Docker verdes. Commit `90e1f59`; publicación remota pendiente. |
