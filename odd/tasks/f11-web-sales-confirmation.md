...
- T11 supplied RED evidence: after migration 0014, database suites failed during `beforeEach` cleanup with PostgreSQL `cannot truncate a table referenced in a foreign key constraint`; no redundant RED rerun was performed.
- T11 focused database group 1: `DATABASE_URL=postgresql://farmaxia:local-development-only@localhost:5433/farmaxia_test pnpm --filter @farmaxia/api exec vitest run --config vitest.config.ts test/sales.spec.ts test/cash.spec.ts test/inventory.spec.ts test/procurement.spec.ts`: PASS (exit code 0; 4 files, 34 tests).
...
## Progress
- Status: T1-T12 implemented in scope; the Web sales workspace now blocks incomplete forms and explains each missing requirement with dashboard navigation, `normalizeSaleInput` has focused pure-unit coverage for valid CASH normalization and invalid input boundaries, PostgreSQL migration/integration verification passes locally, and the API contract documentation matches the implemented F11 endpoint.
...
- T11 completion: appended `cascade` only to the nine affected test-only cleanup statements in auth, cash, catalog, inventory, inventory report, platform services, procurement, subscription quotas, and tenancy RLS fixtures. `transversal-services.spec.ts` already used `cascade`; no additional fixture with the affected cleanup pattern was found. All requested database groups now pass against `farmaxia_test`; no task-specific verification gap remains. The observed `pg` deprecation warning remains a non-blocking follow-up outside this minimal fixture cleanup scope.
...
- Risks: `tenancy.rls.spec.ts` emits a non-blocking `pg` deprecation warning about concurrent `client.query()` calls. T8 has no Web test script available, so confidence comes from TypeScript, production build, diff validation, and code inspection.
...
- T12 commit evidence: `5f90451` (`docs(api): align F11 sales confirmation contract`) plus `8103533` (`docs(api): clarify F11 inventory allocation`) containing only the API contract documentation and the F11 ledger correction.
- Next: F11 implementation and local PostgreSQL verification are complete; delivery remains a separate branch/PR decision.
